...
-- [bids] 제목 풀텍스트 검색 (simple: 한국어 호환)
CREATE INDEX IF NOT EXISTS idx_bids_title_search
  ON bids USING gin (to_tsvector('simple', title));

-- [bids] 기관명 풀텍스트 검색 (simple: 한국어 호환)
CREATE INDEX IF NOT EXISTS idx_bids_organization_search
  ON bids USING gin (to_tsvector('simple', organization));
