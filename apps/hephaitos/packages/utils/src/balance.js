/**
 * @ioblock/utils - Balance Utilities
 ...
 */
export function normalizeBalance(exchange, rawBalances, prices) {
    switch (exchange) {
        case 'binance':
            return normalizeBinance(rawBalances, prices);
        case 'upbit':
            return normalizeUpbit(rawBalances, prices);
        case 'bithumb':
            return normalizeBithumb(rawBalances, prices);
        default:
 ...
 * 바이낸스 잔고 정규화
 */
function normalizeBinance(balances, prices) {
    return balances
 ...
 * 업비트 잔고 정규화
 */
function normalizeUpbit(balances, prices) {
    return balances
 ...
 * 빗썸 잔고 정규화
 */
function normalizeBithumb(balances, prices) {
    return balances
 ...
 */
export function sortByValue(assets) {
    return [...assets].sort((a, b) => b.value_usd - a.value_usd);
}
/**
 * 최소 가치 이하 자산 필터링 (더스트 제거)
 */
export function filterDust(assets, minValueUsd = 1) {
