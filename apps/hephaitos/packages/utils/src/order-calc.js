/**
 * @ioblock/utils - Order Calculation Utilities
 ...
 * 현재 레버리지 계산
 */
export function calculateLeverage(positionValue, equity) {
    if (equity <= 0)
        return 0;
 ...
 * 청산가 계산
 */
export function calculateLiquidationPrice(entryPrice, leverage, side, maintenanceMarginRate = 0.5) {
    if (leverage <= 0)
        return 0;
    // 청산 = 마진 손실 100%
 ...
 * 손절가 계산 (비율 기반)
 */
export function calculateStopLossPrice(entryPrice, side, stopLossPercent) {
    const direction = side === 'buy' ? -1 : 1;
 ...
 * ATR 기반 손절가 계산
 */
export function calculateATRStopLoss(entryPrice, side, atr, multiplier = 2) {
 ...
 * 익절가 계산 (비율 기반)
 */
export function calculateTakeProfitPrice(entryPrice, side, takeProfitPercent) {
    const direction = side === 'buy' ? 1 : -1;
 ...
 * R:R 비율 기반 익절가 계산
 */
export function calculateTakeProfitByRR(entryPrice, stopLossPrice, side, rrRatio) {
 ...
 * 추적 손절가 업데이트
 */
export function updateTrailingStopPrice(currentPrice, currentStopPrice, side, trailingPercent) {
    const direction = side === 'buy' ? -1 : 1;
 ...
 * 손익률 계산 (%)
 */
export function calculatePnLPercent(entryPrice, exitPrice, side) {
    if (entryPrice === 0)
        return 0;
    const direction = side === 'buy' ? 1 : -1;
 ...
 * 미실현 손익 계산
 */
export function calculateUnrealizedPnL(entryPrice, currentPrice, quantity, side) {
 ...
 */
export function calculateAvgEntryPrice(existingQuantity, existingAvgPrice, newQuantity, newPrice) {
 ...
 * R:R 비율 계산
 */
export function calculateRiskRewardRatio(entryPrice, stopLossPrice, takeProfitPrice) {
 ...
 */
export function calculateRMultiple(entryPrice, stopLossPrice, currentPrice, side) {
 ...
 * 주문 유효성 검증
 */
export function validateOrder(request, riskConfig, currentPrice, currentEquity, openPositionCount) {
    const errors = [];
    const warnings = [];
    // 기본 검증
    ...
    if (positionPercent > riskConfig.maxPositionSize) {
        errors.push(`포지션 크기(${positionPercent.toFixed(1)}%)가 최대 허용치(${riskConfig.maxPositionSize}%)를 초과합니다`);
    }
    // 동시 포지션 수 검증
    ...
    if (request.stopLoss?.price) {
        ...
        if (riskPercent > riskConfig.maxRiskPerTrade) {
            errors.push(`거래당 리스크(${riskPercent.toFixed(1)}%)가 최대 허용치(${riskConfig.maxRiskPerTrade}%)를 초과합니다`);
        }
    }
    else {
 ...
 * 슬리피지 시뮬레이션
 */
export function simulateSlippage(requestedPrice, side, slippagePercent, volatilityMultiplier = 1) {
    // 슬리피지는 불리한 방향으로 발생
    const direction = side === 'buy' ? 1 : -1;
 ...
 * 실제 슬리피지 계산
 */
export function calculateSlippage(requestedPrice, executedPrice) {
...
//# sourceMappingURL=order-calc.js.map