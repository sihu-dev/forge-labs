/**
 * @forge/utils - Backtest Calculation Utilities
 ...
 * 낙폭 기록 추출
 */
export function extractDrawdownRecords(
  ...
  for (let i = 0; i < equityCurve.length; i++) {
    ...
    if (point.equity > peak) {
      ...
      if (inDrawdown && currentRecord) {
        ...
        currentRecord.recoveryDays = daysBetween(
          currentRecord.troughTime!,
          point.timestamp
        );
